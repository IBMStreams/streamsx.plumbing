...
// DEBUG: inputPort[0] type is tuple<float64 time,rstring label>
// DEBUG: outputPort[0] type is tuple<float64 time,rstring label>
// DEBUG: inputPort[1] type is tuple<float64 time,rstring label>
// DEBUG: outputPort[1] type is tuple<float64 time,rstring label>
// DEBUG: timeFactor is 'lit$0'




#include "./SynchronizedEvenStream.h"
...
void MY_OPERATOR_SCOPE::MY_OPERATOR::process(Tuple const & tuple, uint32_t port)
{
    ...
    switch(port) {
case 0: { const IPort0Type& iport$0 = tuple; nextTimestamp[0] = static_cast<double>(iport$0.get_time()); } break;
case 1: { const IPort1Type& iport$1 = tuple; nextTimestamp[1] = static_cast<double>(iport$1.get_time()); } break;
...
std::string printCounts(uint32_t* counters) {
   ...
   buffer << "[";
   for (int i=0; i < 2; i++) { buffer << (i==0 ? "" : ",") << counters[i]; }
   buffer << "]";
   return buffer.str();
}
...
{
    for (int i=0; i < 2; i++) { if (  counters[i]<counters[port] ) return false; }
    return true;
}
...
bool MY_BASE_OPERATOR::globalIniter() {
    instantiators_.insert(std::make_pair("SynchronizedEvenStream",&initer));
    return true;
}
...
MY_BASE_OPERATOR::MY_BASE_OPERATOR()
 : Operator() {
    ...
    $oportBitset.push_back(OPortBitsetType(std::string("011")));
    $oportBitset.push_back(OPortBitsetType(std::string("011")));
}
MY_BASE_OPERATOR::~MY_BASE_OPERATOR()
{
