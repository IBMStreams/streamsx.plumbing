...
#ifndef BEJWRMXIZKCNMTS02ZULMSS0BACOHAV4_H_
#define BEJWRMXIZKCNMTS02ZULMSS0BACOHAV4_H_
...
#define SELF BeJwrMXIzKcnMTS02zUlMSs0BACoHAV4
...
public:
    ...
    typedef SPL::float64 time_type;
    typedef SPL::rstring label_type;

    enum { num_attributes = 2 } ;
    
    SELF() : Tuple(), time_(), label_() {}
    SELF(const Self & ot) : Tuple(), time_(ot.time_), label_(ot.label_) 
      { constructPayload(ot); }
    SELF(const time_type & _time, const label_type & _label) : Tuple(), time_(_time), label_(_label) { }
    ...
    time_type & get_time() { return time_; }
    const time_type & get_time() const { return time_; }
    void set_time(const time_type & _time) { time_ = _time; }
    ...
    {
        size_t s = 17;
        s = 37 * s + std::tr1::hash<time_type >()(time_);
...
#undef SELF
#endif // BEJWRMXIZKCNMTS02ZULMSS0BACOHAV4_H_ 
